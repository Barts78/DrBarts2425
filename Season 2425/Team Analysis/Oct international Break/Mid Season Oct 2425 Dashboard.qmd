---
title: "Mid Season Review Oct 2425"
...
Vizlocation <- paste0("~/Dropbox/AI LFC data/UP Pod Prep 2425/Oct International Break/")
...
#set Game name
GameName <- paste("EPL2425GameDatabase - Oct")
...
MatchSummaryHalfData <- MatchSummaryHalfData %>%
  select('Game Half' = Half, 'Team' = Team_Group, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFFT Shots' = OFShots, 'ONT Shots' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % Con.', 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success)
...
MatchSummaryPeriodData <- MatchSummaryPeriodData %>%
  select('Time Period' = Game_Period, 'Team' = Team_Group, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFFT Shots' = OFShots, 'ONT Shots' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % Con.', 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success)

MatchSummaryGameStateData <- plottingData %>%
  mutate(Successful_Touch = ifelse(isTouch == "TRUE", 1, 0)) %>%
  mutate(TeamEPV = ifelse(str_detect(`outcomeType/value`, "1"), paste0(EPVvalue), 0),
         TeamEPV = as.numeric(TeamEPV)) %>%
  group_by(Game_State, Team_Group) %>% 
  summarise_at(vars(Goals, In_play_time, Touches, Final_Third_Touches, TotalShots, OFShots, OTShots, SixYard_Box_Shots, PK_Box_Shots, Out_Box_Shots, BCs, xG, NPxG, xGOT, Attempted_Passes, Successful_Passes, TeamEPV), list(sum), na.rm = TRUE) %>%
  mutate(In_play_time = round((In_play_time/60),1),
         Pass_Success = round((Successful_Passes/Attempted_Passes)*100,1),
         'EPV % Con.' = round((xG/TeamEPV)*100,1),
...
MatchSummaryGameStateData <- MatchSummaryGameStateData %>%
  select('Game State' = Game_State, 'Team' = Team_Group, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFFT Shots' = OFShots, 'ONT Shots' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % Con.', 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success)
...
dashboard_data <- dashboard_data %>%
  ...
  select(x, y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, teamId, TotalShots, xG, NPxG, xGOT, expandedMinute, type, Game_Block, Game_State)
...
B1_Histogram_xG <- df_B1xG %>%
  ...
  labs(title = paste(MC_Title_text),
       subtitle = paste("1000 sims, ", Match_Fixture1),
       x="Goals",
       y="Count", 
       caption = My_caption) +
...
EPVHeatMapplotFT <- EPVHeatMapDataFT %>%
  ...
  #plot player positions
  #geom_point(data = OppoAverage_PlayerFT, aes(x, y), fill = Color_Away, size = 5, shape = 21, colour = "black", stroke = 0.5) +
      
  #plot player names
  #geom_text(data = OppoAverage_PlayerFT, aes(x, y, label = player_shirt), color = Color_Away_Text, size = 2) +
    
#          scale_fill_gradientn(
#          colors = c("#FFFFF0", Color_Home),
         # values = scales::rescale(c(0, 1)), 
          #breaks = scales::breaks_width(0.05), 
          #name = "EPV") +
...
dashboard_data <- dashboard_data %>%
  ...
  mutate(Goal_point_X = as.numeric(Goal_point_X)) %>%
  mutate(Goal_point_Y = as.numeric(Goal_point_Y)) %>%
  select(X = x, Y = y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, teamId, TotalShots, xG, NPxG, xGOT, minute, expandedMinute, result = type, player = playerId, Team_Group, Game_Block, Game_State)
...
LFC_S_plot <- plot_shot_AI(DrawDashboard_data, highlight_goals = TRUE, average_location = FALSE, mapType = HomeHA, mapName = Match_Home, teamColor = Color_Home, matchDate = Game_date, matchFixture = Match_Fixture1, dataSource = "Opta", logo = "", wrap = "")
...
LFC_S_plot <- plot_shot_AI(GTimeDashboard_data, highlight_goals = TRUE, average_location = FALSE, mapType = HomeHA, mapName = Match_Home, teamColor = Color_Home, matchDate = Game_date, matchFixture = Match_Fixture1, dataSource = "Opta", logo = "", wrap = "")
