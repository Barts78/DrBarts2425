---
title: "CL Match Day R16 Leg 2 Season 2425"
...
#set away
Match_Away <- paste0("Paris Saint-Germain")

#set Away color
Color_Away <- paste("royalblue4")
...
#gg_record(
#  dir = file.path(Vizlocation, "MD_recording"), # where to save the recording
#  device = "png", # device to use to save images
#  width = 2500,      # width of saved image
#  height = 2150,     # height of saved image
...
MatchSummaryData <- plottingData %>%
  ...
  group_by(teamId) %>%  
  summarise_at(vars(Goals, In_play_time, Touches, UnSuccessful_Touches, PK_Box_Touches, Final_Third_Touches, Attacking_Half_Touches, Defensive_Half_Touches, Mis_Control, Dispossessed_Total, Dribbles_Total, Dribbles_Won, Dribbles_Lost, TotalShots, OFShots, Blocked_Shot, OTShots, SixYard_Box_Shots, PK_Box_Shots, Out_Box_Shots, xG, NPxG, xGOT, Attempted_Passes, Successful_Passes, UnSuccessful_Passes, Long_Passes, Successful_Long_Passes, BCs, Corners, Short_Corners, Cross_Passes, Successful_Cross, UnSuccessful_Cross, Tackles_Total, Tackles_Won, Tackles_Lost, Fouls_Total, Fouls_Conceeded, Fouls_Received, Aerial_Won, Aerial_Lost, Interceptions_Total, Recoverys_Total, Offside_Won, Offside_Lost), list(sum), na.rm = TRUE) %>%
  mutate(In_play_time = round((In_play_time/60),1),
         ...
         Failed_Pass = round(Attempted_Passes - Successful_Passes, 2),
         ...
         Posession_Control = (Interceptions_Total + Recoverys_Total + (Tackles_Won - Tackles_Lost) + (Aerial_Won - Aerial_Lost) - (Mis_Control + Dispossessed_Total + UnSuccessful_Passes + UnSuccessful_Cross + Dribbles_Lost  + OFShots + Blocked_Shot + Offside_Lost)),
           ...
           #Poss_Gained - Poss_Lost,
         #Poss_Control = (Interceptions_Total + Recoverys_Total + (Tackles_Won - Tackles_Lost) + (Aerial_Won - Aerial_Lost) - (Mis_Control + Dispossessed_Total + (Attempted_Passes - Successful_Passes) + UnSuccessful_Cross + (Dribbles_Total - Dribbles_Won) + OFShots + Blocked_Shot + Offside_Lost))
         ) %>%
    na.omit() 
...
MatchSummaryHalfData <- MatchSummaryHalfData %>%
  mutate(Half = case_when(Half == "FirstHalf" ~ "First Half",
                          ...
                          Half == "FirstPeriodOfExtraTime" ~ "First Half Extra Time",
                          Half == "SecondPeriodOfExtraTime" ~ "Second Half Extra Time",
                          Half == "PenaltyShootout" ~ "Penalty Shootout",
  ...
  summarise_at(vars(Goals, In_play_time, Touches, UnSuccessful_Touches, PK_Box_Touches, Final_Third_Touches, Attacking_Half_Touches, Defensive_Half_Touches, Mis_Control, Dispossessed_Total, Dribbles_Total, Dribbles_Won, Dribbles_Lost, TotalShots, OFShots, Blocked_Shot, OTShots, SixYard_Box_Shots, PK_Box_Shots, Out_Box_Shots, BCs, xG, NPxG, xGOT, Attempted_Passes, Successful_Passes, UnSuccessful_Passes, Long_Passes, Successful_Long_Passes, Corners, Short_Corners, Cross_Passes, Successful_Cross, UnSuccessful_Cross, Tackles_Total, Tackles_Won, Tackles_Lost, Fouls_Total, Fouls_Conceeded, Fouls_Received, Aerial_Won, Aerial_Lost, Interceptions_Total, Recoverys_Total, Offside_Won, Offside_Lost, TeamEPV), list(sum), na.rm = TRUE) %>%
  mutate(In_play_time = round((In_play_time/60),1),
         Pass_Success = round((Successful_Passes/Attempted_Passes)*100,1),
...
MatchSummaryHalfsTable <- MatchSummaryHalfData %>%
  ...
  tab_style(
    ...
    style = list(cell_text(weight = "bold", size = 16))) %>%
  tab_style(
    locations = cells_row_groups(groups = c("First Half", "Second Half", "First Half Extra Time", "Second Half Extra Time", "Penalty Shootout")),
...
MatchSummaryPeriodTable <- MatchSummaryPeriodData %>%
  ...
  tab_style(
    ...
    style = list(cell_text(weight = "bold", size = 16))) %>%
  tab_style(
    locations = cells_row_groups(groups = c("0 - 15", "16 - 30", "31 - 45", "46 - 60", "61 - 75", "76 - 90", "90 +", "Extra Time")),
    style = cell_text(weight = "bold", size = 16, color = "white")) %>%
...
dashboard_data <- dashboard_data %>%
  ...
  select(x, y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, teamId, TotalShots, xG, NPxG, xGOT, expandedMinute, type, period)
...
Long_Passes_Data <- plottingData %>%
  ...
  select(teamId, playerId, x, y, finalX, finalY, C_End_point_X, C_End_point_Y, minute, Successful_Carries, UnSuccessful_Carries, Prog_Carries, Long_Passes, Cross_Passes, Successful_Passes, UnSuccessful_Passes, Prog_Passes, Distance_Metre, Game_Period, period, EPVvalue, xTvalue, Half = period_displayName) %>%
...
Home_Post_DA_Passes <- plottingData %>%
  select(period, period_displayName, teamId, playerId, player_shirt, Prog_Passes, Prog_Carries, Attempted_Passes, Successful_Passes, Successful_Long_Passes, Attempted_Carries, Successful_Carries, x, y, finalX, finalY, Prev_Defensive_Action, EPVvalue, EPVStart, EPVEnd)
...
Home_Post_DA_Passes <- shift_column(data = Home_Post_DA_Passes, columns = "teamId", new_names = "receiver_team", len = 1, up = TRUE)
...
Away_Post_DA_Passes <- shift_column(data = Away_Post_DA_Passes, columns = "teamId", new_names = "receiver_team", len = 1, up = TRUE)
...
dashboard_data <- dashboard_data %>%
  ...
  mutate(Goal_point_X = as.numeric(Goal_point_X)) %>%
  mutate(Goal_point_Y = as.numeric(Goal_point_Y)) %>%
  select(X = x, Y = y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, teamId, TotalShots, xG, NPxG, xGOT, OTShots, minute, expandedMinute, result = type, player = playerId, Game_Period)
