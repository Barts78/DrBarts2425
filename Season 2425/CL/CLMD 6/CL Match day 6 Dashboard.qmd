---
title: "CL Match Day 6 Season 2425"
...
#set league points
League_Points <- 34
AFC_League_Points <- 25 
MC_League_Points <- 23

#set man of the match
HMOM = paste("Miguel Gutiérrez")
AMOM = paste("Andy Robertson")
...
dashboard_data <- dashboard_data %>%
  ...
  select(x, y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, teamId, TotalShots, xG, NPxG, xGOT, expandedMinute, type, Game_State)
...
Awaydashboard_data <- Awaydashboard_data %>%
  ...
  mutate(Goal_point_X = as.numeric(Goal_point_X)) %>%
  mutate(Goal_point_Y = as.numeric(Goal_point_Y)) %>%
  select(x, y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, teamId, TotalShots, xG, NPxG, xGOT, expandedMinute, type, player = playerId, Game_State)
...
AwayPPDAHeatMapplot2H <- AwayPPDAHeatMapData2H %>%
  ...
  #plot heat map
  stat_density_2d_filled(aes(fill = after_stat(level)), geom = "polygon", alpha = 0.5, adjust = 0.5, linewidth = 0.5) +
...
dashboard_data <- dashboard_data %>%
  ...
  mutate(Goal_point_X = as.numeric(Goal_point_X)) %>%
  mutate(Goal_point_Y = as.numeric(Goal_point_Y)) %>%
  select(X = x, Y = y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, teamId, TotalShots, xG, NPxG, xGOT, minute, expandedMinute, result = type, player = playerId, Game_State)
...
```{r home density plot GS}
xGDensityPlot_GS <- dashboard_data %>%
  ggplot(aes(x = X, y = Y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#454545") +
  theme_pitch() +
  geom_segment(aes(y = 21.1, yend = 21.1, x = 0, xend = 100), linewidth = 0.5, colour = "#95A5A6", linetype = "longdash") +
  ...
  coord_flip(xlim = c(50, 100),
                   ylim = c(100, 00)) + 
  #geometrics
  stat_density_2d(aes(fill = after_stat(level), text = paste("xG: ", ..level..)), geom = "polygon", alpha = 0.5, adjust = 1.5, linewidth = 0.5) +
  scale_color_paletteer_c(ColorScale_HomeC) +
  scale_fill_paletteer_c(ColorScale_HomeC) +
  guides(fill = guide_legend(title = "xG Level")) +
  guides(color = guide_legend(title = "xG Level")) +
  ...
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        ...
        legend.key = element_rect(fill = "snow"),
        strip.background = element_rect(fill = Color_Home, color = "black", linetype = "solid", linewidth = 1),
        strip.text = element_text(size = 10, colour = Color_Home_Text))
...
xGDensityPlotAway <- Awaydashboard_data %>%
  ...
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        ...
        legend.key = element_rect(fill = "snow"),
        strip.background = element_rect(fill = Color_Away, color = "black", linetype = "solid", linewidth = 1),
        strip.text = element_text(size = 10, colour = Color_Away_Text))
...
xGDensityPlotAway
```
``` {r away density plot GS}
xGDensityPlotAway_GS <- Awaydashboard_data %>%
  ggplot(aes(x = X, y = Y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#454545") +
  theme_pitch() +
  geom_segment(aes(y = 21.1, yend = 21.1, x = 0, xend = 100), linewidth = 0.5, colour = "#95A5A6", linetype = "longdash") +
