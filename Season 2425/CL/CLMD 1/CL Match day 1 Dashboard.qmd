---
title: "CL Match Day 1 Season 2425"
...
Vizlocation <- paste0("~/Dropbox/AI LFC data/UP Pod Prep 2425/Matchday 4/")
...
#set Away text
Color_Away_Text <- paste("#4DD0E1")

#Set Home
Match_Home <- paste0("Milan")
...
ColorScale_Away <- paste("ggsci::cyan_material") 

#set away color scale con
ColorScale_AwayC <- paste("ggthemes::Blue-Teal")

#set match date
Game_date <- paste("18-09-24")
...
MatchIdFbref <- paste("https://fbref.com/en/matches/54ed5557/Milan-Liverpool-September-17-2024-Champions-League") 

#set WhoScored Match page
MatchIdWhoScored <- paste("https://www.whoscored.com/Matches/1866149/Live/Europe-Champions-League-2024-2025-AC-Milan-Liverpool")
...
#prepare first sub time data (min before sub) GOT TO FOX this as na does not exsist need to replace with 0
...
#load fbref Data
#set files names
filenamefbref1 <- paste0("~/Dropbox/AI LFC data/data sets/fbref/", GameName, "_", GameWeek, "_Passing_EPL2425.xlsx")
filenamefbref2 <- paste0("~/Dropbox/AI LFC data/data sets/fbref/", GameName, "_", GameWeek, "_Posession_EPL2425.xlsx")
filenamefbref3 <- paste0("~/Dropbox/AI LFC data/data sets/fbref/", GameName, "_", GameWeek, "_Defense_EPL2425.xlsx")
filenamefbref4 <- paste0("~/Dropbox/AI LFC data/data sets/fbref/", GameName, "_", GameWeek, "_Misc_EPL2425.xlsx")
filenamefbref5 <- paste0("~/Dropbox/AI LFC data/data sets/fbref/", GameName, "_", GameWeek, "_Summary_EPL2425.xlsx")
filenamefbref6 <- paste0("~/Dropbox/AI LFC data/data sets/fbref/", GameName, "_", GameWeek, "_Shots_EPL2425.xlsx")
#filenamefbref7 <- paste0("~/Dropbox/AI LFC data/data sets/fbref/", GameName, "_", GameWeek, "_Lineups_EPL2425.xlsx")
#filenamefbref8 <- paste0("~/Dropbox/AI LFC data/data sets/fbref/", GameName, "_", GameWeek, "_MatchReport_EPL2425.xlsx")
...
#prepare shot values for main data frame
evaluate_expression <- function(x) {
  parts <- strsplit(x, "\\+")[[1]]
  sum(as.numeric(parts))
}

MD_shot_values <- MD_shots %>%
  mutate(Minute = sapply(Minute, evaluate_expression)) %>%
  arrange(Minute) %>%
  mutate(Minute = Minute - 1,
         Shot = ifelse(Minute < 0, 0, 1),
         Shot_count = cumsum(Shot)) %>%
  mutate(Squad = ifelse(Home_Away == "Away", Match_Away, Match_Home)) 
...
MD_playermins <- plottingData %>%
  select(teamId, playerId, playerNumber, Shirt_Number = player_shirt, role, Mins_Played = minutes_played) %>%
  group_by(playerId, teamId) %>%
  mutate(Mins_Played = as.numeric(Mins_Played)) %>%
  replace_na(list(Mins_Played = 0)) %>%
  summarise(Mins_Played = min(Mins_Played), playerNumber = first(playerNumber), Shirt_Number = first(Shirt_Number),
    role = first(role), .groups = "drop") %>%
select(PlayerId = playerNumber, Mins_Played, role)

MD_lineups <- Playerdata %>%
    left_join(MD_playermins, by = c('PlayerId' = 'PlayerId'), copy = TRUE) %>%
    replace_na(list(Mins_Played = 0)) %>%
    replace_na(list(role = "Substitute")) %>%
  select(Team = teamId, Player = PlayerName, Shirt_Number = jerseyNumber, Mins_Played, role)
...
lineupdataHome <- MD_lineups %>%
  ...
  row_group_order(groups = c("Keeper","Defender","Midfielder", "Attacker", "Substitute")) %>%
...
#reorder data
#team_order <- c(Match_Home, Match_Away)

#MatchSummaryData  <- MatchSummaryData  %>%
#arrange(factor(Team, levels = team_order)) 
...
MD_MC_Plot <- ggplot(long_prob_matrix, aes(x = Home, y = Away, fill = Probability)) +
  geom_tile() +
  geom_text(aes(label = paste(Probability, "%")), size = 5, color = if_else(long_prob_matrix$Probability > 4.5, Color_Away_Text, Color_Home_Text),) +
        scale_fill_gradientn(
        colors = c(Color_Home, Color_Away),
...
Season_MC_Plot <- ggplot(point_probs, aes(x = total_points, y = prob)) +
  ...
  labs(title = "Liverpool Final Predicted Point Totals (1000 Sims)",
...
HomepassPlot1stsubsWD <- plot_passnet_AI(data = plottingData, 
                                   ...
                                   plot_start = 00, plot_finish = firstSubHome, pitch = "", wrap = "", pressmap = "Yes", logo = "", passes = 4)
...
#ggsave(path = Vizlocation, "Home Pass Map EPV 1st Half (Away half passes only) no logo.png", width = 2500, height = 2150, units = "px", dpi = 400)
...
#save EPV pass network
#ggsave(path = Vizlocation, "Home Pass Map EPV 2nd Half (Away half passes only) no logo.png", width = 2500, height = 2150, units = "px", dpi = 400)
```
...
#save EPV pass network 
#ggsave(path = Vizlocation, "Home Pass Map EPV 1st Sub Game (Away half passes only) no logo.png", width = 2500, height = 2150, units = "px", dpi = 400)
```
...
MatchSummaryPLayerData <- plottingData %>%
  ...
  group_by(teamId, playerId, role, player_shirt,  player_image_url) %>%
    summarise(
    across(
      c(Goals, TotalShots, OFShots, SoT = OTShots, Out_Box_Shots, Touches, Defensive_Half_Touches, Attacking_Half_Touches, 
        Final_Third_Touches, PK_Box_Touches, SixYard_Box_Touches, Attempted_Carries, Successful_Carries, Prog_Carries, 
        Final_Third_Carries, Attempted_Passes, Successful_Passes, Long_Passes, Final_Third_Passes, 
        Prog_Passes, TeamEPV, Key_Passes, Assists, Hockey_Assists, BCs, xG, xGOT, Aerial_total, Aerial_Won, 
...
#subset for detailed stats on passing and touches and carries
MatchSummaryPLayerDetailedData <- MatchSummaryPLayerData

#sub set for overall expected and general player stats like shooting assists kp, bc and epv
MatchSummaryPLayerProfileData <- MatchSummaryPLayerData

#data wrangling for general player stat tables
MatchSummaryPLayerData <- MatchSummaryPLayerData %>%
  select(Team = teamId, role, Name = playerId, Mins = minutes_played, '% of Team Possession' = Possession, `% of Team On Ball Time` = On_Ball_Time, Touches = Touches, 'Total Shots' = TotalShots, SoT, 'Out Box Shots' = Out_Box_Shots, BCs, Goals, xG, xGOT,  'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, Assists, 'Key Passes' = Key_Passes, EPV = TeamEPV, '% of Team EPV' = Team_EPV_P, 'Aerials Total' = Aerial_total, 'Aerials Won' = Aerial_Won, 'Aerial Success %' = Aerial_Success, 'Duels Total' = Ground_Duels_Total, 'Duels Won' = Ground_Duels_Won, 'Duel Success' = Duel_Success)


#data wrangling for player profile plots
Player_Summary_plot_data <- MatchSummaryPLayerProfileData %>%
  select(Team = teamId, Name = playerId, role, '% Possession' = Possession, `% On Ball Time` = On_Ball_Time, Touches = Touches, 'Total Shots' = TotalShots, OFShots, SoT, 'Out Box Shots' = Out_Box_Shots, BCs, Goals, xG, xGOT, '% xG' = Team_xG_P, Touches, PK_Box_Touches, 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, Assists, 'Key Passes' = Key_Passes, EPV = TeamEPV, '% EPV' = Team_EPV_P, 'Aerials Total' = Aerial_total, 'Aerials Won' = Aerial_Won, 'Aerial Success %' = Aerial_Success, 'Duels Total' = Ground_Duels_Total, 'Duels Won' = Ground_Duels_Won, 'Duel Success %' = Duel_Success, '% Duels' = Team_Duels_P, '% Passes' = Team_Passes_P, '% Shots' = Team_Shots_P, '% Aerials' = Team_Aerial_P)
...
MD_adv_passing <- MD_adv_passing %>%
  ...
  #na.omit() %>%
  mutate(Recieving_Suc = (round(Successful_Recieved / Passes_Recieved,2))) %>%
...
# sort out Player Passing stats (xT and xA) from fbred for main tables
#data wrangling
...
#Player Possession table for receiving stats (Fbref) - got the data just need to do the extra calculations
#data wrangling
MatchdayTablePosession <- MatchdayTablePosession %>%
  ...
  select(Team, Shirt = Player_Num, `Carries Distance`, `P Carrie Distance`, `% Carries D Prog`)
...
MatchSummaryPLayerDetailedData <- MatchSummaryPLayerDetailedData %>%
  ...
  mutate(`Defensive Half Touches / Min` = Defensive_Half_Touches / minutes_played) %>%
  mutate(`Attacking Half Touches / Min` = Attacking_Half_Touches / minutes_played) %>%
  mutate(`PK Box Touches / Min` = PK_Box_Touches / minutes_played) %>%
  mutate(`F 3rd Touches / Min` = Final_Third_Touches / minutes_played) %>%
  mutate(`% T D Half` = Defensive_Half_Touches / Touches) %>%
  mutate(`% T A Half` = Attacking_Half_Touches / Touches) %>%
...
#Player Details stats table - just got to sort this out and we are done
MatchdayPlayerTable_All <- MatchSummaryPLayerDetailedData %>%
  select(Team = teamId, full_name = playerId, Position = role, Mins = minutes_played, `Total` = Touches, `Def Half` = Defensive_Half_Touches, `Att Half` = Attacking_Half_Touches, `Final 3rd` = Final_Third_Touches, `PK Box` = PK_Box_Touches, `Total / Min` = `T / Min`, `Def Half % TT` = `% T D Half`, `Att Half % TT` = `% T A Half`, `Final 3rd % TT` = `% T F 3rd`, `PK Box % TT` = `% T PK Box`, `Carries` = Attempted_Carries, `% Carries Prog` = `% P Carries`, `% Carries F 3rd` = `% F 3rd Carries`, `% Carries D Prog`, xT, xA,xG, PSxG = xGOT, `Long Ps`  = Long_Passes, `Final 3rd P` = Final_Third_Passes, `Prog Ps` = Prog_Passes, `P Received` = Passes_Recieved, `PPs Received` = Prog_Passes_Recieved, `KPs Recieved` = Key_Passes_Recieved, `Recieveing Suc` = Recieving_Suc, `EPV Recievedz` = EPV_Recieved)
...
MatchSummaryPlayerTableHome <- MatchSummaryPLayerData %>%
  ...
  tab_source_note(md(My_caption)) %>%
  cols_hide(Team) %>%
  summary_rows(columns = c(5:27), fns = list(Average = ~ mean (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals = 1))) %>%
  summary_rows(columns = c(5:27), fns = list(Total = ~ sum (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals = 0))) %>%
  fmt_percent(
    columns = c(5:6, 17, 21, 24, 27),
    scale_values = FALSE,
    decimals = 1
  ) %>%
  fmt_number(
  ...
  tab_style(
    style = list(cell_text(weight = "bold", color = Color_Home_Text), cell_fill(color = Color_Home)),
    locations = cells_row_groups(groups = everything())) %>%
  tab_spanner(label = "Shots", columns = c(8:14)) %>%
  tab_spanner(label = "Passes", columns = c(15:21)) %>%
  tab_spanner(label = "Duels", columns = c(22:27)) %>%
  tab_style(
    locations = cells_column_spanners('Shots'),
    style = list(cell_text(weight = "bold", size = 16))) %>%
  tab_style(
  ...
  gt_color_rows(columns = c(6, 7, 15, 16, 17, 21, 24, 27), palette = ColorScale_Home, domain = c(0,100)) %>%
  gt_color_rows(columns = c(8, 22, 26), palette = ColorScale_Home, domain = c(0,10)) %>%
  gt_color_rows(columns = c(10:12, 18:19), palette = ColorScale_Home, domain = c(0,5)) %>%
  gt_color_rows(columns = c(23, 25), palette = ColorScale_Home, domain = c(0,20)) %>%
  gt_color_rows(columns = c(20), palette = ColorScale_Home, domain = c(0,1)) %>%
...
MatchSummaryPlayerTableAway <- MatchSummaryPLayerData %>%
  ...
  gt_color_rows(columns = c(6, 7, 15, 16, 17, 21, 24, 27), palette = ColorScale_Away, domain = c(0,100)) %>%
  gt_color_rows(columns = c(8, 22, 26), palette = ColorScale_Away, domain = c(0,10)) %>%
  gt_color_rows(columns = c(10:12, 18:19), palette = ColorScale_Away, domain = c(0,5)) %>%
  gt_color_rows(columns = c(23, 25), palette = ColorScale_Away, domain = c(0,20)) %>%
  gt_color_rows(columns = c(20), palette = ColorScale_Away, domain = c(0,1)) %>%
...
Home_player_stats <- 
  ...
  fmt_percent(columns = c(11:14, 16:18, 29), decimals = 0) %>%
  fmt_number(columns = c(10), decimals = 2) %>%
  cols_align(align = "right", columns = everything()) %>%
  tab_spanner(label = "Touches (T) Breakdown", columns = c(5:10)) %>%
  tab_spanner(label = "% of Total T Thirds Breakdown", columns = c(11:14)) %>%
  tab_spanner(label = "Carries Breakdown", columns = c(15:18)) %>%
  tab_spanner(label = "Expected Metrics", columns = c(19:22)) %>%
  tab_spanner(label = "Passes Breakdown", columns = c(23:30)) %>%
  tab_header(
    title = paste0(Match_Home, " Player Match Detailed Summary"),
    subtitle = paste0(Match_Fixture)) %>%
  tab_source_note(md(My_caption)) %>%
  summary_rows(columns = c(5:30), fns = list("Pos Average" = ~ mean (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals = 1))) %>%
  summary_rows(columns = c(5:30), fns = list("Pos Total" = ~ sum (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals = 0))) %>%
  tab_style(
    style = list(cell_text(weight = "bold", color = Color_Home_Text), cell_fill(color = Color_Home)),
    locations = cells_row_groups(groups = everything())) %>%
  tab_style(
  ...
  tab_options(
    ...
    table.width = "100%") %>%
  tab_style(locations = cells_title(groups = "title"),style = list(cell_text(weight = "bold", size = 16))) %>%
  gt_color_rows(columns = c(10, 20:22), palette = ColorScale_Home, domain = c(0,3)) %>% # expected
  gt_color_rows(columns = c(19), palette = ColorScale_Home, domain = c(0,1.5)) %>% #xT
  gt_color_rows(columns = c(11, 12, 13, 14, 16:18, 29, 30), palette = ColorScale_Home, domain = c(0,1)) %>% # percentage based 
  gt_color_rows(columns = c(26, 28), palette = ColorScale_Home, domain = c(0,10)) %>% #passes
  gt_color_rows(columns = c(7, 8, 9, 15, 22:27), palette = ColorScale_Home, domain = c(0,70)) %>% #passes received and touch breakdown & carries
  gt_color_rows(columns = c(5,6), palette = ColorScale_Home, domain = c(0,100)) # total t
Home_player_stats
...
Away_player_stats <- 
  ...
  tab_options(
    ...
    table.width = "100%") %>%
  tab_style(locations = cells_title(groups = "title"),style = list(cell_text(weight = "bold", size = 16))) %>%
  gt_color_rows(columns = c(10, 20:22), palette = ColorScale_Away, domain = c(0,3)) %>% # expected
  gt_color_rows(columns = c(19), palette = ColorScale_Away, domain = c(0,1.5)) %>% #xT
  gt_color_rows(columns = c(11, 12, 13, 14, 16:18, 29, 30), palette = ColorScale_Away, domain = c(0,1)) %>% # percentage based 
  gt_color_rows(columns = c(26, 28), palette = ColorScale_Away, domain = c(0,10)) %>% #passes
  gt_color_rows(columns = c(7, 8, 9, 15, 22:27), palette = ColorScale_Away, domain = c(0,70)) %>% #passes recieved and touch breakdown & carries
  gt_color_rows(columns = c(5, 6), palette = ColorScale_Away, domain = c(0,100)) # total t
...
AwayEPVTimePlot <- AwayEPVPlotData %>%
  ...
  #geometrics
#  geom_smooth(size = 1.5, colour = Color_Away) +
...
HomeDefEPVTable_Final <- HomeDefEPVTable %>%
  ...
  cols_align(align = "right", columns = everything()) %>%
  cols_label_with(fn = ~ janitor::make_clean_names(., case = "title")) %>%
  row_group_order(groups = c("Defender","Midfielder", "Attacker")) %>%
  fmt_percent(
    columns = c(4),
    scale_values = TRUE,
    decimals = 2
...
EPVHeatMapplotFT <- EPVHeatMapDataFT %>%
  ...
  #geometrics
  stat_density_2d_filled(aes(color = after_stat(level), fill = after_stat(level)), geom = "polygon", alpha = 0.6, adjust = 0.1, linewidth = 1.5) +

  # add zone percentage numbers
  geom_text(data = average_epvNZFT, aes(x = x, y = y, label = paste(PEPV, "%")), size = 5) +
    
  #plot player positions
  geom_point(data = AwayAverage_PlayerFT, aes(x, y), fill = Color_Away, size = 5, shape = 21, colour = "black", stroke = 0.5) +
...
AwayEPVHeatMapplotFT <- AwayEPVHeatMapDataFT %>%
  ...
  # add zone percentage numbers
  geom_text(data = Awayaverage_epvNZFT, aes(x = x, y = y, label = paste(PEPV, "%")), size = 5) +
...
EPVHeatMapplot1sub <- EPVHeatMapData1sub %>%
  ...
  #plot player positions
  geom_point(data = AwayAverage_Player1Sub, aes(x, y), fill = Color_Away, size = 5, shape = 21, colour = "black", stroke = 0.5) +
...
EPVHeatMapplotP1sub <- EPVHeatMapDataP1sub %>%
  ...
  # add zone percentage numbers
  geom_text(data = average_epvNZP1sub, aes(x = x, y = y, label = paste(PEPV, "%")), size = 5) +
    
  #plot player positions
  geom_point(data = AwayAverage_PlayerP1Sub, aes(x, y), fill = Color_Away, size = 5, shape = 21, colour = "black", stroke = 0.5) +
...
AwayEPVHeatMapplot1sub <- AwayEPVHeatMapData1sub %>%
  ...
  # add zone percentage numbers
  geom_text(data = Awayaverage_epvNZ1sub, aes(x = x, y = y, label = paste(PEPV, "%")), size = 5) +
  
  #plot player positions
  geom_point(data = Average_Player1Sub, aes(x, y), fill = Color_Home, size = 5, shape = 21, colour = "black", stroke = 0.5) +
...
AwayEPVHeatMapplotP1sub <- AwayEPVHeatMapDataP1sub %>%
  ...
  # add zone percentage numbers
  geom_text(data = Awayaverage_epvNZP1sub, aes(x = x, y = y, label = paste(PEPV, "%")), size = 5) +
  
  #plot player positions
  geom_point(data = Average_PlayerP1Sub, aes(x, y), fill = Color_Home, size = 5, shape = 21, colour = "black", stroke = 0.5) +
...
EPVHeatMapplot1H <- EPVHeatMapData1H %>%
  ...
  # add zone percentage numbers
  geom_text(data = average_epvNZ1H, aes(x = x, y = y, label = paste(PEPV, "%")), size = 5) +
    
  #plot player positions
  geom_point(data = AwayAverage_Player1H, aes(x, y), fill = Color_Away, size = 5, shape = 21, colour = "black", stroke = 0.5) +
...
EPVHeatMapplot2H <- EPVHeatMapData2H %>%
  ...
  # add zone percentage numbers
  geom_text(data = average_epvNZ2H, aes(x = x, y = y, label = paste(PEPV, "%")), size = 5) +
    
  #plot player positions
  geom_point(data = AwayAverage_Player2H, aes(x, y), fill = Color_Away, size = 5, shape = 21, colour = "black", stroke = 0.5) +
...
AwayEPVHeatMapplot1H <- AwayEPVHeatMapData1H %>%
  ...
  # add zone percentage numbers
  geom_text(data = Awayaverage_epvNZ1H, aes(x = x, y = y, label = paste(PEPV, "%")), size = 5) +
  
  #plot player positions
  geom_point(data = Average_Player1H, aes(x, y), fill = Color_Home, size = 5, shape = 21, colour = "black", stroke = 0.5) +
...
AwayEPVHeatMapplot2H <- AwayEPVHeatMapData2H %>%
  ...
  # add zone percentage numbers
  geom_text(data = Awayaverage_epvNZ2H, aes(x = x, y = y, label = paste(PEPV, "%")), size = 5) +
  
  #plot player positions
  geom_point(data = Average_Player2H, aes(x, y), fill = Color_Home, size = 5, shape = 21, colour = "black", stroke = 0.5) +
...
HomePPDAHeatMapplotFT <- HomePPDAHeatMapDataFT %>%
  ...
  #formatting
  labs(title = paste0(Match_Home," Open Play Defensive Actions Heat Map - Full Game"), 
       subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher, Defensive Actions (DA), Box figures = % of Total DA"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
        panel.background = element_rect(colour = "snow", fill = "snow"),
...
AwayPPDAHeatMapplotFT <- AwayPPDAHeatMapDataFT %>%
  ...
  labs(title = paste(Match_Away," Open Play Defensive Actions Heat Map - Full Game"), 
       subtitle = paste0(Match_Fixture,"  \nIncreased Colour = Higher, Defensive Actions, Box figures = % of Total PPDA"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
        panel.background = element_rect(colour = "snow", fill = "snow"),
...
HomePPDAHeatMapplot1sub <- HomePPDAHeatMapData1sub %>%
  ...
  # add zone percentage numbers
  geom_text(data = average_PPDANZ1sub, aes(x = x, y = y, label = paste(PPDA, "%")), size = 5) +
...
HomePPDAHeatMapplotP1sub <- HomePPDAHeatMapDataP1sub %>%
  ...
  # add zone percentage numbers
  geom_text(data = average_PPDANZP1sub, aes(x = x, y = y, label = paste(PPDA, "%")), size = 5) +
...
AwayPPDAHeatMapplot1sub <- AwayPPDAHeatMapData1sub %>%
  ...
  # add zone percentage numbers
  geom_text(data = Awayaverage_PPDANZ1sub, aes(x = x, y = y, label = paste(PPDA, "%")), size = 5) +
...
AwayPPDAHeatMapplotP1sub <- AwayPPDAHeatMapDataP1sub %>%
  ...
  # add zone percentage numbers
  geom_text(data = Awayaverage_PPDANZP1sub, aes(x = x, y = y, label = paste(PPDA, "%")), size = 5) +
...
# Calculate average player position
#Average_Player1H <- HomePPDAHeatMapData1H %>% 
#  group_by(playerId, player_shirt) %>% 
#  summarise(x = mean(x, na.rm = TRUE), y = mean(y, na.rm = TRUE), events = n()) %>% 
#        na.omit() %>%
#        mutate(x = (100 - x),
...
# Calculate average player position
#Average_Player2H <- HomePPDAHeatMapData2H %>% 
#  group_by(playerId, player_shirt) %>% 
#  summarise(x = mean(x, na.rm = TRUE), y = mean(y, na.rm = TRUE), events = n()) %>% 
#        na.omit() %>%
#        mutate(x = (100 - x),
...
HomePPDAHeatMapplot1H <- HomePPDAHeatMapData1H %>%
  ...
  # add zone percentage numbers
  geom_text(data = average_PPDANZ1H, aes(x = x, y = y, label = paste(PPDA, "%")), size = 5) +
...
HomePPDAHeatMapplot2H <- HomePPDAHeatMapData2H %>%
  ...
  # add zone percentage numbers
  geom_text(data = average_PPDANZ2H, aes(x = x, y = y, label = paste(PPDA, "%")), size = 5) +
...
AwayPPDAHeatMapplot1H <- AwayPPDAHeatMapData1H %>%
  ...
  # add zone percentage numbers
  geom_text(data = Awayaverage_PPDANZ1H, aes(x = x, y = y, label = paste(PPDA, "%")), size = 5) +
...
AwayPPDAHeatMapplot2H <- AwayPPDAHeatMapData2H %>%
  ...
  # add zone percentage numbers
  geom_text(data = Awayaverage_PPDANZ2H, aes(x = x, y = y, label = paste(PPDA, "%")), size = 5) +
...
#xGLinePlot_I <- ggplotly(xGLinePlot, tooltip = "text")

#xGLinePlot_I <- xGLinePlot_I %>%
