---
title: "Match Day 10 Season 2425"
...
Vizlocation <- paste0("~/Dropbox/AI LFC data/UP Pod Prep 2425/Matchday 10/")

#set away
Match_Away <- paste0("Brighton & Hove Albion")

#set Away color
Color_Away <- paste("#436EEE")
...
ColorScale_Away <- paste("ggsci::light_blue_material")

#set away color scale con
ColorScale_AwayC <- paste("ggthemes::Blue")

#set match date
Game_date <- paste("02-11-24")
...
MatchIdWhoScored <- paste("https://www.whoscored.com/Matches/1821253/Live/England-Premier-League-2024-2025-Liverpool-Brighton")
...
Playerdata <- Playerdata %>%
  filter(role == "Substitute") %>%
    select(Team = teamId, Player = PlayerName, Shirt_Number = jerseyNumber, Mins_Played = minutes_played, role)
...
#Playerdata <- Playerdata %>%
#  filter(role == "Substitute") %>%
#    select(teamId, playerId = PlayerName, Shirt_Number = jerseyNumber, minutes_played, role)

#MD_lineups <- MD_lineups %>%
#  left_join(Playerdata, by = c('Team' = 'Team'), copy = TRUE)
...
MatchSummaryTable <- MatchSummaryData %>%
  ...
  tab_spanner(label = "Possession", columns = c(2:5)) %>%
