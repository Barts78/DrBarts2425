...
Vizlocation <- paste0("~/Dropbox/AI LFC data/UP Pod Prep 2425/Match day 3/")

#set Away
Match_Away <- paste0("Liverpool")

#set Away color
Color_Away <- paste("white")

#set Away text
Color_Away_Text <- paste("#d3171e")
...
ColorScale_AwayC <- paste("ggthemes::Light Grays")

#set match date
Game_date <- paste("01-09-24")
...
MatchIdFbref <- paste("https://fbref.com/en/matches/a7ab7a12/North-West-Derby-Manchester-United-Liverpool-September-1-2024-Premier-League") 

#set WhoScored Match page
MatchIdWhoScored <- paste("https://www.whoscored.com/Matches/1821070/Live/England-Premier-League-2024-2025-Manchester-United-Liverpool")
...
# prepare line up data for main data frame
MD_logos <- MD_lineups %>%
  mutate(Pos = case_when(Pos == "FW" ~ "Attacker",
                         ...
                         Pos == "GK" ~ "Keeper",
                         Pos == "LW,LB" ~ "Defender",
                         Pos == "RM,AM" ~ "Midfielder",
                         Pos == "LM,FW" ~ "Midfielder",
                         Pos == "WB,RM" ~ "Defender",
                         Pos == "AM,RM" ~ "Midfielder",
                         Pos == "WB,LB" ~ "Defender",
                         Pos == "CB,CM" ~ "Defender",
                         Pos == "LM,CM" ~ "Midfielder",
                         Pos == "LM,RM,CM" ~ "Midfielder",
                         Pos == "LM,RM,CM" ~ "Midielder",
                         Pos == "RW,LW,LM" ~ "Midielder",
                         Pos == "RW,DM" ~ "Midfielder",
                         Pos == "CB,RM" ~ "Defender",
                         Pos == "LM,RM" ~ "Midfielder",
                         Pos == "WB,AM" ~ "Defender",
                         Pos == "RB,RW" ~ "Defender",
                         TRUE ~ as.character(Pos))) %>%
                        replace_na(list(Pos = "Substitute")) %>%
                        replace_na(list(Min = 0)) %>%
  mutate(Team = ifelse(Home_Away == "Away", Match_Away, Match_Home)) %>%
  select(Team, Player = Player_Name, player_shirt = Player_Num, role = Pos, "stats_minutes_played" = Min, image_url = PlayerURL) %>%
  mutate(player_shirt = as.numeric(player_shirt))
...
lineupdata <- MD_logos %>%
  select(team_name = Team, Player, image_url, Shirt_Number = player_shirt, role, Mins_Played = stats_minutes_played) %>%
