---
title: "Match day 35 Season 2425"
...
compute_summary <- function(data, group_vars) {
  data %>%
    ...
    summarise(across(c(Goals, In_play_time, Touches, UnSuccessful_Touches, PK_Box_Touches,
                       ...
                       Recoverys_Total, Offside_Won, Offside_Lost, TeamEPV, Defensive_Third_Touches, 
...
#Set Home
Match_Home <- paste0("Chelsea")

#set Home color
Color_Home <- paste("blue3")
...
#ColorScale_Home <- scale_fill_manual(
#  values = c(Color_Home_Text, Color_Home))

#set home color scale con
ColorScale_HomeC <- paste("ggthemes::Blue")

#ColorScale_HomeC <- scale_fill_gradientn(
#          colors = c(Color_Home_Text, Color_Home))

#set match date
Game_date <- paste("04-05-25")
...
Vizlocation <- paste0("~/Dropbox/AI LFC data/R files/AI UP/Scripts/Viz Pack/Season 2425/", GameWeek)
...
MatchSummaryTable <- MatchSummaryData %>%
  ...
  tab_spanner(label = "Passes", columns = c(18:24)) %>%
  tab_style(
...
MatchSummaryHalfsTable <- MatchSummaryHalfData %>%
  ...
  tab_spanner(label = "Passes", columns = c(19:25)) %>%
  tab_style(
...
plot_passnet_AI <- function(data, team_name, scale_stat = "EPV", 
    ...
    if ((nrow(data) > 0) &&
        sum(c("x", "y", "finalX", "finalY", "TeamName", "playerId", "type", "expandedMinute", "outcome", "Game_Period", "player_shirt") %in% names(data)) == 11) {
    } else {
      print(c("x", "y", "finalX", "finalY", "TeamName", "playerId", "type", "expandedMinute", "outcome", "Game_Period", "player_shirt"))
      ...
      if (pressmap == "Yes") {
        ...
        #formatting
          labs(title = PassNetTitle_text,
             ...
             x = "Direction of Attack Right to Left",
             colour = leg_title,
             caption = My_caption) +
        ...
        PressData <- PressData %>%
          filter(Defensive_Action == 1)
            ...
            plotExplaination <- paste0(
              glue("<span style='font-family:\"Arial\";color:gray12;font-size:10px;'>Thicker Line = More Passes</span><span style='font-family:\"Arial\";color:gray12;font-size:10px;'>, Darker Circle Colour = Higher EPV</span><span style='font-family:\"Arial\";color:gray12;font-size:10px;', Darker Line = Higher Pass Completion, Direction of Attack Right to Left</span>"))
...
HomepassPlot1stWD <- plot_passnet_AI(data = plottingData,
                                  ...
                                  plot_start = 00, plot_finish = 44, pressmap = "Yes", logo = "")
HomepassPlot1stWD
...
HomepassPlot2ndWD <- plot_passnet_AI(data = plottingData,  
                                  ...
                                  plot_start = 47, plot_finish = 100, pressmap = "Yes", logo = "")
HomepassPlot2ndWD
...
AwaypassPlot2ndWD <- plot_passnet_AI(data = plottingData,  
                                   ...
                                   plot_start = 46, plot_finish = 100, pressmap = "Yes", logo = "")
AwaypassPlot2ndWD
...
HomepassPlot1stsubsWD <- plot_passnet_AI(data = plottingData, 
                                   ...
                                   plot_start = 00, plot_finish = firstSubHome, pitch = "", wrap = "", pressmap = "Yes", logo = "")
...
HomepassPlotPost1stsubsWD <- plot_passnet_AI(data = plottingData, 
                                        ...
                                        plot_start = firstSubHome, plot_finish = 100, pitch = "", wrap = "", pressmap = "Yes", logo = "")
...
AwaypassPlot1stsubsWD <- plot_passnet_AI(data = plottingData, 
                                   ...
                                   plot_start = 00, plot_finish = firstSubAway, pitch = "", wrap = "", pressmap = "Yes", logo = "")
...
AwaypassPlotpost1stsubsWD <- plot_passnet_AI(data = plottingData, 
                                        ...
                                        plot_start = firstSubAway, plot_finish = 100, pitch = "", wrap = "", pressmap = "Yes", logo = "")
...
HomepassPlot1subAtt <- plot_passnet_AI(data = plottingData,  
                                    ...
                                    plot_start = 00, plot_finish = firstSubHome, pitch = "Half", pressmap = "Yes")
HomepassPlot1subAtt
...
HomepassPlotFTAttW <- plot_passnet_AI(data = plottingData,  
                                      ...
                                      plot_start = 00, plot_finish = 100, pitch = "Half", pressmap = "Yes", wrap = "Yes", passes = 1)
HomepassPlotFTAttW
```
...
Zone14_Passes_Home_Data <- plottingData %>%
  ...
  select(TeamName, playerId, x, y, finalX, finalY, C_End_point_X, C_End_point_Y, Successful_Carries, UnSuccessful_Carries, Prog_Carries, Successful_Passes, UnSuccessful_Passes, Prog_Passes, Distance_Metre, Game_Period, EPVvalue, Zone_14_Finish, Zone_14_Start)
...
LHS_Passes_Home_Data <- plottingData %>%
  ...
  select(TeamName, playerId, x, y, finalX, finalY, C_End_point_X, C_End_point_Y, Successful_Carries, UnSuccessful_Carries, Prog_Carries, Successful_Passes, UnSuccessful_Passes, Prog_Passes, Distance_Metre, Game_Period, EPVvalue, Left_HS_Finish, Left_HS_Start)
...
RHS_Passes_Home_Data <- plottingData %>%
  ...
  select(TeamName, playerId, x, y, finalX, finalY, C_End_point_X, C_End_point_Y, Successful_Carries, UnSuccessful_Carries, Prog_Carries, Successful_Passes, UnSuccessful_Passes, Prog_Passes, Distance_Metre, Game_Period, EPVvalue, Right_HS_Finish, Right_HS_Start)
...
#save Plot
ggsave(path = Vizlocation, "Home Z14 and HS plot.png", width = 2500, height = 2150, units = "px", dpi = 400)
```
...
#save Plot
ggsave(path = Vizlocation, "Away Z14 and HS plot.png", width = 2500, height = 2150, units = "px", dpi = 400)
```
## Row {.flow}
```{r}
#| out-height: 100%
...
Long_Passes_Data <- plottingData %>%
  ...
  select(TeamName, playerId, x, y, finalX, finalY, C_End_point_X, C_End_point_Y, minute, Successful_Carries, UnSuccessful_Carries, Prog_Carries, Long_Passes, Cross_Passes, Successful_Passes, UnSuccessful_Passes, Prog_Passes, Distance_Metre, Game_Period, period, EPVvalue, xTvalue, Half = period_displayName) %>%
...
#from fbref
#MatchSummaryPLayerData <- MatchSummaryPLayerData %>%
#  left_join(MatchdayTablePass, by = c("TeamName" = "Team", "player_shirt" = "Shirt"), copy = TRUE) %>% 
#  mutate_all(~ ifelse(is.na(.), 0, .))

#MatchSummaryPLayerData <- compute_summary(plottingData, group_vars = c("playerId", "TeamName"))
...
MatchSummaryPLayerStandardData <- MatchSummaryPLayerStandardData %>%
  select(Team = TeamName, role, Name = playerId, Mins = minutes_played, '% of Team Possession' = Possession, `% of Team On Ball Time` = On_Ball_Time, 'Total Shots' = TotalShots, SoT, 'Out Box Shots' = Out_Box_Shots, Goals, xG, xGOT, xA = MyxA, xT = TeamxT,  'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, Assists, BCs, 'Key Passes' = Key_Passes, `Long Ps`  = Long_Passes, `Final 3rd P` = Final_Third_Passes, `Prog Ps` = Prog_Passes, 'PK Box P' = PK_Box_Passes, `P Received` = Passes_Recieved, `PPs Received` = Prog_Passes_Recieved, `KPs Recieved` = Key_Passes_Recieved, `Recieveing Suc` = Recieving_Suc, `EPV Recieved` = EPV_Recieved, 'EPV created' = TeamEPV, '% of Team EPV' = Team_EPV_P)

#data wrangling for detailed player stats table 2
MatchSummaryPLayerDetailedData <- MatchSummaryPLayerDetailedData %>%
  select(Team = TeamName, full_name = playerId, Position = role, Mins = minutes_played, Touches, `Def 3rd` = Defensive_Third_Touches, `Mid 3rd` = Middle_Third_Touches, `Final 3rd` = Final_Third_Touches, `PK Box` = PK_Box_Touches, `Total / Min` = `T / Min`, `Def 3rd % TT` = `% T D Third`, `Mid Third % TT` = `% T M Third`, `Final 3rd % TT` = `% T F 3rd`, `PK Box % TT` = `% T PK Box`, `Carries` = Successful_Carries, `% Carries Prog` = `% P Carries`, `% Carries F 3rd` = `% F 3rd Carries`, `% Carries D Prog`, 'Aerials Total' = Aerial_total, 'Aerials Won' = Aerial_Won, 'Aerial Success %' = Aerial_Success, 'Duels Total' = Ground_Duels_Total, 'Duels Won' = Ground_Duels_Won, 'Duel Success' = Duel_Success,'Possession Control' = Posession_Control)
...
PassConnections <- PassConnections %>%
  filter(type == "Pass") %>% 
  select(Passer = playerId, Receiver = receiver, Passer_Team = TeamName, Receiver_Team = receiver_team, Successful_Passes, BCs, Key_Passes, Prog_Passes, EPV = EPVvalue, x, y, Endx, Endy) 
...
```{r}
# Enhanced TouchPlayerPlot_AI with method selection and dynamic faceting
TouchPlayerPlot_AI <- function(
  ...
  data <- data %>%
    filter(!`qualifiers/0/type/value` %in% c(2, 5, 6, 107),
           !`qualifiers/1/type/value` %in% c(2, 5, 6, 107),
           !`qualifiers/2/type/value` %in% c(2, 5, 6, 107),
           !`qualifiers/3/type/value` %in% c(2, 5, 6, 107),
  ...
  shape_fn <- if (method == "mad") touch_area_shape_mad else touch_area_shape
...
HometouchPlot1Sub <- TouchTeamPlot_OPTA(HomeTeamTouchData,  title = paste0(Match_Home, " Team On Ball Touch"),
                                     plot_start = 00, plot_finish = firstSubHome, logo = "", method = "mad",
...
AwaytouchPlot1Sub <- TouchTeamPlot_OPTA(AwayTeamTouchData,  title = paste0(Match_Away, " Team On Ball Touch"),
                                       plot_start = 00, plot_finish = firstSubAway, method = "mad",
                                       subtitle = paste0(Match_Fixture, " Up to 1st Sub"))
AwaytouchPlot1Sub
...
EPVPlotData <- EPVPlotData %>%
  ...
  select(TeamName, expandedMinute, minute, type, Home_Cumalative_EPV, TeamEPV, EPVvalue, Goal_point_Y, Goal_point_X, Cumalative_EPV, Home_Cumalative_EPV)
...
AwayEPVPlotData <- AwayEPVPlotData %>%
  ...
  select(TeamName, expandedMinute, minute, type, Away_Cumalative_EPV, TeamEPV, EPVvalue, Goal_point_Y, Goal_point_X, Cumalative_EPV, Away_Cumalative_EPV)
```
...
DefEPVData <- DefEPVData %>%
  filter(Defensive_Action == 1) %>%
  select(TeamName, playerId, EPVvalue, Defensive_Action, outcome, Def_EPV, role) %>%
...
dashboard_data <- dashboard_data %>%
  ...
  mutate(Goal_point_X = as.numeric(Goal_point_X)) %>%
  mutate(Goal_point_Y = as.numeric(Goal_point_Y)) %>%
  select(X = x, Y = y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, TeamName, TotalShots, xG, NPxG, xGOT, OTShots, minute, expandedMinute, result = type, player = playerId, Game_Period)
...
``` {r}
#gg_playback(
#  dir = file.path(Vizlocation, "MD_recording"),
#  name = "~/Dropbox/AI LFC data/R files/AI UP/Scripts/Viz Pack/Season 2425/MD20/MD_Recording/MD20.gif",
