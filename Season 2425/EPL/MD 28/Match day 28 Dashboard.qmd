---
title: "Match Day 28 Season 2425"
...
#set away
Match_Away <- paste0("Southampton")

#set Away color
Color_Away <- paste("yellow")

#set Away text
Color_Away_Text <- paste("royalblue4")
...
HMOM = paste("Harvey Elliott")
AMOM = paste("Tyler Dibling")
...
Long_Passes_Data <- plottingData %>%
  ...
  mutate(Pass_Type = ifelse(Cross_Passes == 1, "Cross",
    ...
    mutate(Half = ifelse(period == 1, "First Half",
                            ifelse(period == 2, "Second Half", "Injury Time")))
...
dashboard_data <- dashboard_data %>%
  ...
  mutate(Goal_point_X = as.numeric(Goal_point_X)) %>%
  mutate(Goal_point_Y = as.numeric(Goal_point_Y)) %>%
  select(X = x, Y = y, Goal_point_Y, Goal_point_X, TotalxG, TotalNPxG, TotalxGOT, situation, teamId, TotalShots, xG, NPxG, xGOT, OTShots, minute, expandedMinute, result = type, player = playerId)
...
plot_shot_AI <- function(data, type = "", teamColor = "#B22222", text_color = "", bins = 30, highlight_goals = "", average_location = "", matchFixture = "", mapName = "", mapType = "", matchDate = "", dataSource = "", logo = "", wrap = "") {
  ...
  # Checking data frame   
  if ((nrow(data) > 0) &&
      sum(c("minute", "X", "Y", "xG", "xGOT", "NPxG", "result", "OTShots", "player") %in% names(data)) == 9) {
