---
title: "Match Day 23 Season 2425"
...
Vizlocation <- paste0("~/Dropbox/AI LFC data/R files/AI UP/Scripts/Viz Pack/Season 2425/MD23/")

#set away
Match_Away <- paste0("Ipswich Town")

#set Away color
Color_Away <- paste("royalblue3")
...
#set league points
League_Points <- 53
AFC_League_Points <- 47
MC_League_Points <- 38

#set man of the match
HMOM = paste("Trent Alexander-Arnold")
AMOM = paste("Kalvin Phillips")
...
Long_Passes_Away_Data <- plottingData %>%
  filter(teamId == Match_Away,
         Long_Passes == 1 | Cross_Passes == 1 | Successful_Passes == 1 | Successful_Carries == 1) %>%
  select(teamId, playerId, x, y, finalX, finalY, C_End_point_X, C_End_point_Y, minute, Successful_Carries, UnSuccessful_Carries, Prog_Carries, Long_Passes, Cross_Passes, Successful_Passes, UnSuccessful_Passes, Prog_Passes, Distance_Metre, Game_Period, period, EPVvalue, xTvalue, Half = period_displayName)  %>%
  mutate(Pass_Type = ifelse(Cross_Passes == 1, "Cross",
                            ifelse(Long_Passes == 1, "Long Pass","Successful"))) %>%
    mutate(Half = ifelse(period == 1, "First Half",
                            ifelse(period == 2, "Second Half", "Injury Time"))) %>%
  filter(Pass_Type != "Successful")
...
EPVHeatMapplotFTW <- EPVHeatMapDataFTW %>%
  ...
  #geometrics
  stat_density_2d(aes(fill = after_stat(level)), geom = "polygon", alpha = 0.5, adjust = 0.25, linewidth = 0.5) +
...
HomePostPPDAHeatMapplotFT <- HomePPDAHeatMapDataFT %>%
  ...
  geom_point(data = Home_Post_DA_Passes_1st, aes(x = finalX, y = finalY, color = Prog_Action), fill = Color_Home, size = 6, shape = 21, stroke = 0.5, alpha = 0.8) +
...
AwayPostPPDAHeatMapplotFT <- AwayPPDAHeatMapDataFT %>%
  ...
  #plot recieving player from DA
  geom_point(data = Away_Post_DA_Passes_1st, aes(x = finalX, y = finalY, color = Prog_Action), fill = Color_Away, size = 6, shape = 21, stroke = 0.5, alpha = 0.8) +
...
HomePostPPDAHeatMapplotFT <- HomePPDAHeatMapDataFT %>%
  ...
  #plot recieving player from DA
  geom_point(data = Home_Post_DA_Passes_2nd, aes(x = finalX, y = finalY, color = Prog_Action), fill = Color_Home, size = 6, shape = 21, stroke = 0.5, alpha = 0.8) +
...
AwayPostPPDAHeatMapplotFT <- AwayPPDAHeatMapDataFT %>%
  ...
  #plot recieving player from DA
  geom_point(data = Away_Post_DA_Passes_2nd, aes(x = finalX, y = finalY, color = Prog_Action), fill = Color_Away, size = 6, shape = 21, stroke = 0.5, alpha = 0.8) +
